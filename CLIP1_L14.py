# ============================
# FAST TRAIN (frames preprocessati) - ViT-L/14
...
def predict_batch(clip_model, head, text_bank, imgs, alpha_sup=0.7):
    # ----- CAMBIO 2: niente .float() -----
    z = F.normalize(clip_model.encode_image(imgs), dim=-1)
    logits_sup = head(z)  # lasciare in AMP
...
def train_and_eval():
    ...
    clip_model.float()                      # << forza i parametri in FP32
    ...
    # class weights per sbilanciamento 80/20
    counts = Counter([it["label"] for it in collect_frames(DATA_DIR)])
    total = sum(counts.values())
    class_weights = torch.tensor([total/(2*counts[0]), total/(2*counts[1])], device=DEVICE)
    ce = nn.CrossEntropyLoss(weight=class_weights if not USE_WEIGHTED_SAMPLER else None)


    head = LinearHead(embed_dim, 2).to(DEVICE).float()
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # ----- CAMBIO 4: checkpoint leggero (solo bias MLP + head) -----
        tuned_bias_state = {
    ...
    # ----- CAMBIO 4: ricarica dei soli bias e della head -----
    ...
    try:
        auc_roc = roc_auc_score(y_true, prob_fake)
    except ValueError:
        auc_roc = float("nan")
