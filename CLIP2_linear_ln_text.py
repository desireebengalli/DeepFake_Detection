...
RESULTS_DIR = "/home/liciabordignion/results2/CLIP2_linear_ln_with_text"
RESULTS_CSV_METRICS = os.path.join(RESULTS_DIR, "clip2_test_metrics_global_linear_ln_with_text.csv")
RESULTS_CSV_METRICS_VIDEO = os.path.join(RESULTS_DIR, "clip2_test_metrics_video_linear_ln_with_text.csv")
SAVE_PATH = os.path.join(RESULTS_DIR, "clip2_linear_ln_with_text.pt")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for step, (imgs, y) in enumerate(pbar, 1):
            ...
            with autocast_ctx():
                ...
                logits_sup = head(z)
                logits_txt = clip_model.logit_scale.exp() * (z @ text_bank.t())
                loss = alpha_sup * ce(logits_sup, y) + (1 - alpha_sup) * ce(logits_txt, y)
        ...
        if improved:
            ...
            epochs_no_improve += 1
            print(f"No improvement of AUC video. epochs_no_improve = {epochs_no_improve}/{PATIENCE}")
