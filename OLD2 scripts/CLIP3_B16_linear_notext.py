# ============================
# FAST TRAIN (frames preprocessati) - ViT-B/16 (VISUAL-ONLY)
# Linear head + PEFT (bias-tuning su MLP del vision encoder)
# Cosine LR con warmup, gradient accumulation, loss ibrida (CE + contrastiva)
# TEST:
...
#   - RIMOSSA ogni dipendenza dai textual embeddings/prompts di CLIP
#   - Niente text_bank, niente logit_scale, niente mixing alpha_sup
#   - Training e inferenza SOLO dal ramo visivo + linear head
# ============================
...
# ---------- CONFIG per T4 ----------
MODEL_NAME = "ViT-B/16"     # visual-only
EPOCHS = 10
BATCH_SIZE = 16
...
RESULTS_DIR = "/home/giadapoloni/results/CLIP3_B16_linear_notext"
RESULTS_CSV_METRICS = os.path.join(RESULTS_DIR, "clip3_test_metrics_global_notext.csv")
RESULTS_CSV_METRICS_VIDEO = os.path.join(RESULTS_DIR, "clip3_test_metrics_video_notext.csv")
SAVE_PATH = os.path.join(RESULTS_DIR, "clip3_linear_notext.pt")
...
@torch.no_grad()
def predict_batch_visual_only(clip_model, head, imgs):
...
def train_and_eval():
    ...
    # 2) Porta tutto in FP32 per stabilità
    clip_model.float()
    ...
    # mantenere il visual in eval (niente dropout/bn), head in train
    ...
    for imgs, labels, paths in tqdm(test_loader, desc="Testing (frames)"):
        imgs = imgs.to(DEVICE)
        with autocast_ctx():
            logits = predict_batch_visual_only(clip_model, head, imgs)
        ...
        for pth, lab, pr in zip(paths, labels, batch_prob_fake):
            ...
            # tieni massimo 32 frame per video
