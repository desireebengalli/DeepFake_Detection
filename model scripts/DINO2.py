...
RESULTS_DIR = "/home/default/resultsDINO2"
RESULTS_CSV_METRICS = os.path.join(RESULTS_DIR, "dino2_test_metrics_global_linear_ln.csv")
RESULTS_CSV_METRICS_VIDEO = os.path.join(RESULTS_DIR, "dino2_test_metrics_video_linear_ln.csv")
SAVE_PATH = os.path.join(RESULTS_DIR, "dino2_linear_ln.pt")
...
def build_optimizer_and_scheduler(head, projection, tuned_params, total_steps):
    params = (
        list(head.parameters())
        + list(projection.parameters())
        + [p for p in tuned_params if p.requires_grad]
    )
...
def dino_extract_features(backbone, imgs, projection):
    tokens = backbone.get_intermediate_layers(imgs, n=1)[0]
    cls_tok = tokens[:, 0]
    patch_tok = tokens[:, 1:]
    patch_mean = patch_tok.mean(dim=1)
    concat_feat = torch.cat([cls_tok, patch_mean], dim=-1)
    feats = projection(concat_feat)
    feats = F.normalize(feats, dim=-1)
    return feats

@torch.no_grad()
def dino_predict_batch(backbone, projection, head, imgs):
    z = dino_extract_features(backbone, imgs, projection)
    logits = head(z)
    return logits

@torch.no_grad()
def evaluate(backbone, projection, head, data_loader, device, video_threshold=0.5, verbose=False):
    backbone.eval()
    projection.eval()
    head.eval()
    ...
    for imgs, labels, paths in tqdm(data_loader, desc="Eval", leave=False):
        imgs = imgs.to(device)
        with autocast_ctx():
            logits = dino_predict_batch(backbone, projection, head, imgs)
...
def train_and_eval():
    ...
    backbone, projection = get_dino_backbone(DEVICE)
    ...
    with torch.no_grad():
        ...
        dummy_feats = dino_extract_features(backbone, dummy, projection)
    ...
    optimizer, scheduler = build_optimizer_and_scheduler(head, projection, tuned_params, total_steps)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        frame_m, video_m = evaluate(
            backbone, projection, head, test_loader, DEVICE,
