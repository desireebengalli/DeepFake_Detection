...
# LRs scheduler:warmup + cosine decay
BASE_LR = 3e-4             # LR max
MIN_LR = 1e-5              # LR min
...
RESULTS_DIR = "/home/liciabordignion/results_linearprobing"
RESULTS_CSV_METRICS = os.path.join(RESULTS_DIR, "clip0_test_metrics_global_linear.csv")
RESULTS_CSV_METRICS_VIDEO = os.path.join(RESULTS_DIR, "clip0_test_metrics_video_linear.csv")
SAVE_PATH = os.path.join(RESULTS_DIR, "clip0_linear.pt")
...
def build_optimizer_and_scheduler(head, total_steps):
    """
    Optimizer + scheduler 
    """
...
def train_and_eval():
    ...
    optimizer, scheduler = build_optimizer_and_scheduler(head, total_steps)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        print(f"\nValidation on test after epoch {epoch}...")
    ...
    print(f"Saving CSV (frame) in {RESULTS_CSV_METRICS}")
    ...
    if video_metrics is not None:
        ...
        print(f"Saving CSV (video) in {RESULTS_CSV_METRICS_VIDEO}")
    else:
        print("No video found.")
