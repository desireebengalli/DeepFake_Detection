...
def slerp_within_class(z, y, t_min=0.0, t_max=1.0, eps=1e-7):
    z = F.normalize(z, dim=-1)
...
def train_and_eval():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for step, (imgs, y) in enumerate(pbar, 1):
            ...
            # add SLERP augmentation
            with autocast_ctx():
                z = extract_features(clip_model, imgs)
                z, y = slerp_within_class(z, y)
                logits = head(z)
                loss = ce(logits, y)
    ...
    print("Reloading best checkpoint and metrics...")
